...
void setup() {
  ...
  catEeking = loadImage("catEeking.png");
  ...
  text("girl, are you a bird", 450, 150);
  text("cuz you make me wanna", 450, 175);
  text("ekekekekk", 450, 200);
...
// Step 8: Write the message you would like on the button to open the card
  text("open card!", 450, 280);
  
// draw your image(s)
  image(catEeking, 30, 50, 300, 300); 
}
...
// Step 9: Write the message you would like on the button to close the card
    ...
    // Add "valentine?" text
    fill(#FF8DA1);  // White color for text
    textSize(24);
    textAlign(CENTER, CENTER);
...
void mousePressed() {
  if (buttonClicked) {
    ...
    if (mouseX > 400 && mouseX < 500 && mouseY > 260 && mouseY < 300) {
      buttonClicked = true;
    }
  }
